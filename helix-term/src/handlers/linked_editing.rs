...
mod tests {
    ...
    #[test]
    fn update_ranges_preserves_bounds_at_edit_boundaries() {
        let text = Rope::from("<foo></foo>");
        let insertion = Transaction::change(&text, [(7, 7, Some(Tendril::from("x")))].into_iter());
        let mut ranges = vec![Range::new(1, 4), Range::new(7, 10)];

        update_ranges(&mut ranges, insertion.changes());

        assert_eq!(ranges, vec![Range::new(1, 4), Range::new(7, 11)]);

        let replacement = Transaction::change(
            &Rope::from("<foo></xfoo>"),
            [(7, 11, Some(Tendril::from("foo")))].into_iter(),
        );

        update_ranges(&mut ranges, replacement.changes());

        assert_eq!(ranges, vec![Range::new(1, 4), Range::new(7, 10)]);
    }
